  ...
  checkAllowedUser = async () => {
    ...
    if (userData.id in config.config.access.users) {
      ...
      return {
        ...
        discordId: this.preferredDiscordServer,
        debugInfo
      }
    }
    ...
    for (const guildId of discordsToCheck) {
      ...
      if (config.config.access.discords[guildId].acl && config.config.access.discords[guildId].acl !== 'none') {
        highestAccess.push({
          user: guildInfo.user.username,
          acl: config.config.access.discords[guildId].acl,
          discordId: guildId,
        })
      }
    }
